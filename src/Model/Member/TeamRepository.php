...
class TeamRepository extends ReadOnlyRepository implements ObjectRepository
{
    ...
    public function createQueryBuilder($alias = null)
    {
        ...
        $oQuery = new TeamQueryBuilder($this->getEntityManager()->getConnection(), $this);
    ...
    public function find($id)
    {
        ...
        $result = $qb->where($this->getAlias() . '.team_id = :iTeamId')
            ->setParameter('iTeamId', $id)
            ->execute()
            ->fetch();
     ...
     * Find all teams 
     * 
     * @return object|null the rest collection
     */ 
    public function findAllTeams()
    {
        $qb = $this->getLoadQuery();
        $sAlias = $this->getAlias();       
        $result = $qb->orderByTeamNameAsc($sAlias)
                     ->execute()
